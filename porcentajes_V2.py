    ...
    with col2:
        ...
        if file2:
            try:
                ...
                st.success("✅ Cargado")
                st.dataframe(df_mat, use_container_width=True, height=150)
            except Exception as e: st.error(f"Error: {e}")
        else: st.info("Esperando archivo...")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        if file3:
            try:
                ...
                st.success("✅ Cargado")
                st.dataframe(df_cli, use_container_width=True, height=150)
            except Exception as e: st.error(f"Error: {e}")
        else: st.info("Esperando archivo...")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col4:
        ...
        if file4:
            try:
                ...
                st.success("✅ Cargado")
                st.dataframe(df_dem, use_container_width=True, height=150)
            except Exception as e: st.error(f"Error: {e}")
        else: st.info("Esperando archivo...")
        st.markdown('</div>', unsafe_allow_html=True)
